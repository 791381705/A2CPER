...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        K = self.W_k(x)
        V = self.W_v(x)
        attention_scores = torch.matmul(Q, K.transpose(1, 2)) / torch.sqrt(torch.tensor(self.embed_dim))
...
class Policy(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.softmax(action_score.view(-1,2), dim=-1), state_value.view(-1,1)
...
def plot_durations(episode_durations, average_length,average_reward):
    ...
    path = './A2CPER_CartPole-v1-text_time/' + 'A2CPER_RunTime' + str(RunTime) + '.jpg'
