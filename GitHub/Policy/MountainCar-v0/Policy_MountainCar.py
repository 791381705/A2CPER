...
def plot(steps):
    ax = plt.subplot(111)
    ax.cla()
    ax.set_title('Policy_Mountain Time')
    ax.set_xlabel('Episode')
    ax.set_ylabel('Run Time')
    ax.plot(steps)
    RunTime = len(steps)
    path =  './PG_MountainCar-v0-time/'+'RunTime'+str(RunTime)+'.jpg'
    if len(steps) % 200 == 0:
        plt.savefig(path)
    plt.pause(0.0000001)

def save_table(steps, episode):
    plt.figure(figsize=(6, 4))
    plt.plot(steps)
    plt.xlabel('Steps')
    plt.ylabel('Rewards')
    plt.title('Episode {}'.format(episode))
    path = './PG_MountainCar-v0-time/' + 'Episode' + str(episode) + '_Table.jpg'
    plt.savefig(path)
    plt.close()
