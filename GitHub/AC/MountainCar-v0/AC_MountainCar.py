...
class AC(nn.Module):
    def __init__(self):
        ...
        self.gamma = GAMMA
        os.makedirs('AC_MountainCar-v0_time/', exist_ok=True)
...
def plot(steps):
    ...
    ax.cla()
    ax.grid()
    ax.set_title('AC_Mountain Time')
    ax.set_xlabel('Episode')
    ax.set_ylabel('Run Time')
    steps = np.minimum(steps, 10000)  # 将超过10000的数据限制为10000
    ax.plot(steps)
    plt.ylim(0, 10000)  # 直接设置纵坐标的上限为 10000
    RunTime = len(steps)
    path = './AC_MountainCar-v0_time/' + 'RunTime' + str(RunTime) + '.jpg'
    if len(steps) % 200 == 0:
        plt.savefig(path)
    plt.pause(0.0000001)
...
def main():
    ...
    for i_episode in range(NUM_EPISODES):
        ...
        if i_episode % 100 == 0 and i_episode !=0:
            ...
            modelPath = './AC_MountainCar-v0_time/ModelTraing' + str(i_episode) + 'Times.pkl'
