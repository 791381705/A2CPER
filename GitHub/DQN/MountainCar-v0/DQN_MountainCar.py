...
class Dqn():
    ...
    def plot(self, ax, x):
        ...
        if len(x) % 200 == 0:
            ...
            plt.xlabel('Episode')
            plt.ylabel('Run Time')
            plt.title('DQN_Mountain Time')
            path = './DQN_mountain_car_v1_time/' + 'Time_Taken_episode' + str(len(x)) + '.jpg'
            plt.savefig(path)
            plt.close()
    ...
    for episode in range(EPISODES):
        ...
        while True:
            ...
            if net.memory_counter >= MEMORY_CAPACITY:
                ...
                if done:
                    print(
                        "episode {}, the reward is {}, time taken = {}".format(episode, round(reward, 3), step_counter))
